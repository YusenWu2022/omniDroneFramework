...
class CapsuleCapsuleColliderFnTest(absltest.TestCase):
  ...
  def test_collision(self):
    ...
    capsules = engine.physics.geometry.Capsule(config.bodies, sys.body)
    capsule1 = jp.take(capsules, 0)
    capsule2 = jp.take(capsules, 1)
    qp1 = jp.take(sys.default_qp(), 0)
    qp2 = jp.take(sys.default_qp(), 1)

    contact = engine.physics.colliders.capsule_capsule(capsule1, capsule2, qp1,
                                                     qp2)
...
class BoxHeightMapColliderFnTest(absltest.TestCase):
  ...
  def test_collision_position(self):
    ...
    config = text_format.Parse(BoxHeightMapColliderFnTest._CONFIG,
                               engine.Config())
    sys = engine.System(config)
    box_collider = engine.physics.geometry.BoxCorner([jp.take(config.bodies, 0)],
                                                   sys.body)
    box_collider = jp.take(box_collider, 0)
    heightmap_collider = engine.physics.geometry.HeightMap(
    ...
    contact = engine.physics.colliders.box_heightmap(box_collider,
...
class CapsuleMeshTest(absltest.TestCase):
  ...
  def test_collision_position(self):
    ...
    config = text_format.Parse(CapsuleMeshTest._CONFIG, engine.Config())
    sys = engine.System(config)
    ...
    mesh = engine.physics.geometry.Mesh(
        jp.array([jp.take(config.bodies, 1)]), sys.body, mesh_geoms)
    capsule1 = jp.take(capsules, 0)
    mesh = jp.take(mesh, 0)
    qp = sys.default_qp(0)
    contact = engine.physics.colliders.capsule_mesh(capsule1,
                                                  mesh, jp.take(qp, 0),
                                                  jp.take(qp, 1))
