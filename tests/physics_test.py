...
import omnidrone.engine as engine
from omnidrone.engine import jumpy as jp
import jax
...
class BoxTest(absltest.TestCase):
  ...
  def test_box_hits_ground(self):
    ...
    sys = engine.System(text_format.Parse(BoxTest._CONFIG, engine.Config()))
    qp = sys.default_qp(0)
    qp, _ = sys.step(qp, jp.array([]))
...
class BoxCapsuleTest(absltest.TestCase):
  ...
  def test_box_hits_capsule(self):
    ...
    sys = engine.System(text_format.Parse(BoxCapsuleTest._CONFIG, engine.Config()))
    qp = sys.default_qp()
...
class HeightMapTest(absltest.TestCase):
  ...
  def test_box_stays_on_heightmap(self):
    ...
    sys = engine.System(text_format.Parse(HeightMapTest._CONFIG, engine.Config()))
    qp = sys.default_qp()
...
class SphereTest(absltest.TestCase):
  ...
  def test_sphere_hits_ground(self):
    ...
    sys = engine.System(text_format.Parse(SphereTest._CONFIG, engine.Config()))
    qp = sys.default_qp(0)
    qp, _ = sys.step(qp, jp.array([]))
...
class CapsuleTest(absltest.TestCase):
  ...
  def test_capsule_hits_ground(self):
    ...
    sys = engine.System(text_format.Parse(CapsuleTest._CONFIG, engine.Config()))
    qp = sys.default_qp(0)
    qp, _ = jax.jit(sys.step)(qp, jp.array([]))
  ...
  def test_capsule_hits_capsule(self):
    ...
    config = text_format.Parse(CapsuleTest._CONFIG, engine.Config())
    config.dt = 2.0
    config.substeps = 400
    sys = engine.System(config, engine.Config())
    qp = sys.default_qp(1)
    qp, _ = jax.jit(sys.step)(qp, jp.array([]))
    self.assertAlmostEqual(qp.pos[0, 2], 0.5, 2)  # standing up and down
...
class MeshTest(absltest.TestCase):
  ...
  def test_mesh_hits_ground(self):
    ...
    config = text_format.Parse(MeshTest._CONFIG, engine.Config())
    resource_path = os.path.join(absltest.get_default_test_srcdir(),
                                 'brax/tests/testdata')
    sys = engine.System(config, [resource_path])
    qp = sys.default_qp()
...
class JointTest(parameterized.TestCase):
  ...
  def test_pendulum_period(self, mass, radius, vel):
    ...
    config = text_format.Parse(JointTest._CONFIG, engine.Config())
  ...
  def test_reference_offset(self, offset, axis, limit):
    ...
    for l in range(3):
      ...
      sys_default = engine.System(config)
      ...
      sys_offset = engine.System(rotated_config)
...
class Actuator1DTest(parameterized.TestCase):
  ...
  def test_1d_angle_actuator(self, target_angle):
    ...
    config = text_format.Parse(Actuator1DTest._CONFIG, engine.Config())
    sys = engine.System(config=config)
    qp = sys.default_qp()
...
class Actuator2DTest(parameterized.TestCase):
  ...
  def test_2d_angle_actuator(self, target_angle_1, target_angle_2):
    ...
    config = text_format.Parse(Actuator2DTest._CONFIG, engine.Config())
    sys = engine.System(config=config)
    qp = sys.default_qp()
...
class Actuator3DTest(parameterized.TestCase):
  ...
  def test_3d_torque_actuator(self, limits, torques):
    ...
    config = text_format.Parse(Actuator3DTest._CONFIG, engine.Config())
    for t in torques:
...
class SphericalizeTest(parameterized.TestCase):
  ...
  def test_free_dofs(self):
    ...
    config = text_format.Parse(SphericalizeTest._CONFIG, engine.Config())
    sys = engine.System(config)
...
class ForceTest(parameterized.TestCase):
  ...
  def test_thruster(self, force):
    """A simple part actuates to a target angle."""
    config = text_format.Parse(ForceTest._CONFIG, engine.Config())
    sys = engine.System(config=config)
    qp = sys.default_qp()
...
class ElasticityTest(parameterized.TestCase):
  ...
  def test_ball_bounce(self, elasticity):
    ...
    config = text_format.Parse(ElasticityTest._CONFIG, engine.Config())
